...
def main():
    ...
    output_file = os.path.join(DATA_DIR, f"results_wdegp_{FUNCTION_NAME}.json")
...
def single():
    ...
    outfile = os.path.join(DATA_DIR, f"results_wdegp_{FUNCTION_NAME}.json")

    print(f"  WDEGP — Morris — n_train={n_train}, seed={seed}")
    result = run_single(n_train, seed)
