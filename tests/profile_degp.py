"""
Profile DEGP optimization to identify bottlenecks.
Uses Morris 20D: n_train=20 (=DIM), all points with all gradients.
"""
...
# DEGP: all first-order partial derivatives at all points
der_indices = [[[[i + 1, 1]]] for i in range(DIM)]
derivative_locations = [list(range(N_TRAIN)) for _ in range(DIM)]

# y_train: [func_values, partial_dim1, ..., partial_dim20]
y_train = [y_vals.reshape(-1, 1)] + [grads[:, j:j+1] for j in range(DIM)]

print(f"Building DEGP model: n_train={N_TRAIN}, DIM={DIM}")
model = degp(
    X_train, y_train,
