"""
Benchmark: JetGP DEGP on the 10D active subspace function.
Full gradient-enhanced GP using all 10 partial derivatives.
...
def main():
    ...
    for n_train in SAMPLE_SIZES:
        print(f"\n{'='*60}")
        print(f"  JetGP DEGP — Active Subspace 10D — n_train = {n_train}")
        print(f"{'='*60}")
    ...
    output_file = os.path.join(DATA_DIR, f"results_jetgp_degp_{FUNCTION_NAME}.json")
...
def single():
    ...
    outfile = os.path.join(DATA_DIR, f"results_jetgp_degp_{FUNCTION_NAME}.json")
