"""
Benchmark: GEKPLS (SMT) on the Borehole function (8D)
Gradient-enhanced kriging with partial least squares.
CPU only, single thread for fair comparison.

Same as gekpls_borehole.py but with extra_points=DIM instead of 1.
...
DIM = 8
SAMPLE_SIZES = [DIM, 5 * DIM, 10 * DIM]  # 8, 40, 80
N_MACROREPLICATES = 5
N_TEST = 2000
N_COMP = DIM
...
def run_single(n_train, seed):
    """
    Run a single GEKPLS benchmark with extra_points=DIM.
    """
...
def main():
    ...
    for n_train in SAMPLE_SIZES:
        print(f"\n{'='*60}")
        print(f"  GEKPLS (extra_points={EXTRA_POINTS}) — Borehole — n_train = {n_train}")
        print(f"{'='*60}")
    ...
    # Save results
    output_file = os.path.join(DATA_DIR, f"results_gekpls_ndim_{FUNCTION_NAME}.json")
