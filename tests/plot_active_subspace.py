"""
Benchmark comparison plots for the 10D active subspace experiment.
Compares DDEGP (2 directions), DEGP (10 partials), GEKPLS, GPyTorch.

Usage:
    python plot_active_subspace.py --data_dir /path/to/json/files --out_dir /path/to/output
"""
...
gekpls_file      = f'results_gekpls_{FUNC}.json'
gekpls_ndim_file = f'results_gekpls_ndim_{FUNC}.json'
gpt_files   = {it: f'results_gpytorch_{FUNC}_{it}iter.json' for it in SHOW_ITERS}

COLORS = {
    'DDEGP (2 dirs)':  '#1D9E75',
    'DEGP (10 partials)': '#3A5E3A',
    'GEKPLS':          '#D85A30',
    'GEKPLS (ndim)':   '#E8944A',
    'GPyTorch (100)':  '#C6DCEF',
    'GPyTorch (200)':  '#9ECAE1',
...
def build_summary(metric):
    ...
    for n, v in get_agg(load(ddegp_file), metric).items():
        if n in summary: summary[n]['DDEGP (2 dirs)'] = v

    for n, v in get_agg(load(degp_file), metric).items():
        if n in summary: summary[n]['DEGP (10 partials)'] = v
...
def make_plot(metric, ylabel, fname, invert=True):
    ...
    fig.suptitle(
        f'Active Subspace 10D \u2014 {ylabel} (log scale)'
...
# ── Run ───────────────────────────────────────────────────────────────────────
make_plot('nrmse',      'NRMSE',           'as_nrmse.pdf',      invert=True)
make_plot('train_time', 'Train time (s)',   'as_train_time.pdf', invert=True)
print('All done.')
