...
def main():
    ...
    for n_dirs in N_DIRS_LIST:
        ...
        output_file = os.path.join(DATA_DIR, f"results_jetgp_gddegp_{n_dirs}dirs_{FUNCTION_NAME}.json")
...
def single():
    n_dirs = int(sys.argv[2])
    n_train = int(sys.argv[3])
    seed = int(sys.argv[4])
    rep = int(sys.argv[5]) if len(sys.argv) > 5 else 1
    outfile = os.path.join(DATA_DIR, f"results_jetgp_gddegp_{n_dirs}dirs_{FUNCTION_NAME}.json")
    print(f"  JetGP GDDEGP ({n_dirs} dirs) — {FUNCTION_NAME} — n_train={n_train}, seed={seed}")
    result = run_single(n_train, seed, n_dirs)
