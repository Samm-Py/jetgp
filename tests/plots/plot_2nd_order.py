"""
Benchmark comparison plots for 2nd-order derivative-enhanced GP.
Compares JetGP DEGP (n_order=2) vs GPyTorch RBFKernelGradGrad across
borehole (8D), OTL circuit (6D), and Morris (20D).
Reads JSON from data/; saves figures to figures/.
"""
...
def build_summary(func, metric):
    ...
    # JetGP 2nd order
    jetgp_recs = load(f'results_jetgp_2nd_order_{func}.json')
    if jetgp_recs:
        for n, v in get_agg(jetgp_recs).items():
            if n in summary:
                summary[n]['JetGP (2nd)'] = v

    # GPyTorch 2nd order at each iteration count
    for it in SHOW_ITERS:
        gpt_recs = load(f'results_gpytorch_2nd_order_{func}_{it}iter.json')
        if gpt_recs:
            label = f'GPyTorch ({it})'
            for n, v in get_agg(gpt_recs).items():
                if n in summary:
                    summary[n][label] = v
...
def make_grid(metric, ylabel, fname, invert=True):
    ...
    col_labels = [r'$n = d$', r'$n = 3d$', r'$n = 5d$']
    ...
    fig.suptitle(
        f'2nd-Order GP: {ylabel} (log scale) \u2014 Mean $\\pm$ std over 5 macroreplicates',
        fontsize=13, y=1.01)
    plt.tight_layout()
    out = out_dir / fname
...
# ── Run ──────────────────────────────────────────────────────────────────────
make_grid('nrmse',      'NRMSE',                       '2nd_order_nrmse_grid.pdf',      invert=True)
make_grid('train_time', 'Train time (s)',               '2nd_order_time_grid.pdf',       invert=False)
make_grid('total_time', 'Train + prediction time (s)',  '2nd_order_total_time_grid.pdf', invert=False)
print('All done.')
