"""
Benchmark comparison plots for JetGP paper (R1.2).
Usage:
    python plot_grids_final.py --data_dir /path/to/json/files --out_dir /path/to/output

All JSON files are expected to be in the same directory (data_dir).
"""
...
parser.add_argument('--data_dir', type=str, default='.',
                    help='Directory containing all JSON result files')
...
gpt_files = {
    'borehole': {
        100:  'results_gpytorch_borehole_100iter.json',
...
gekpls_files = {
    'borehole':    'results_gekpls_borehole.json',
    'otl_circuit': 'results_gekpls_otl_circuit.json',
    'morris':      'results_gekpls_morris.json',
}
gekpls_ndim_files = {
    'borehole':    'results_gekpls_ndim_borehole.json',
    'otl_circuit': 'results_gekpls_ndim_otl_circuit.json',
    'morris':      'results_gekpls_ndim_morris.json',
}
func_meta = {
...
def build_summary(func, metric):
    ...
    for n, v in get_agg(load(gekpls_files[func])).items():
        if n in summary: summary[n]['GEKPLS'] = v

    # GEKPLS with extra_points=DIM (optional — skip if file not found)
    ndim_file = gekpls_ndim_files.get(func)
    if ndim_file and (data_dir / ndim_file).exists():
        for n, v in get_agg(load(ndim_file)).items():
            if n in summary: summary[n]['GEKPLS (ndim)'] = v

    for it in SHOW_ITERS[func]:
        label = f'GPyTorch ({it})'
        for n, v in get_agg(load(gpt_files[func][it])).items():
