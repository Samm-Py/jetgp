"""
Benchmark comparison plots: WDEGP vs DEGP on Morris 20D.
Also includes GEKPLS, GEKPLS (ndim), and GPyTorch for completeness.

Loads:
  - DEGP from results_jetgp_morris.json
  - WDEGP from results_wdegp_morris.json
  - GEKPLS from results_gekpls_morris.json
  - GEKPLS (ndim) from results_gekpls_ndim_morris.json
  - GPyTorch from results_gpytorch_morris_*iter.json

Usage:
    python plot_wdegp_morris.py --data_dir /path/to/json/files --out_dir /path/to/output
"""
...
# ── Run ───────────────────────────────────────────────────────────────────────
make_plot('nrmse',      'NRMSE',          'wdegp_morris_nrmse.pdf',      invert=True)
make_plot('train_time', 'Train time (s)', 'wdegp_morris_train_time.pdf', invert=False)
print('All done.')
