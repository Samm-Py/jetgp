"""
Benchmark: GPyTorch DEGP on the 10D active subspace function.
First-order gradient-enhanced GP with RBFKernelGrad (SE equivalent), ARD.
Adam optimizer, all 10 partial derivatives used in training.
"""
...
def main():
    ...
    for n_train in SAMPLE_SIZES:
        print(f"\n{'='*60}")
        print(f"  GPyTorch DEGP — Active Subspace 10D — n_train = {n_train}")
        print(f"{'='*60}")
    ...
    output_file = f"results_gpytorch_{FUNCTION_NAME}_{N_TRAINING_ITER}iter.json"
