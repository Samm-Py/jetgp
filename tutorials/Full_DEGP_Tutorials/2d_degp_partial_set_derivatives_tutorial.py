...
def run_degp_experiment(X_train: np.ndarray, der_indices: List, n_order: int, 
    ...
    try:
        ...
        opt_start = time.time()
        params = gp.optimize_hyperparameters(n_restart_optimizer=15, swarm_size=100)
...
def main():
    ...
    if len(successful_results) > 0:
        print(f"{'Strategy':<20} {'NRMSE':<12} {'Time(s)':<10} {'Obs':<8} {'Efficiency'}")
        print("-" * 70)
        
        for strategy_name in ['gradient_only', 'main_derivatives', 'with_mixed', 'complete']:
    ...
    if main_result['success']:
        ...
        print(f"  Efficiency: {main_result['total_obs']} observations from {num_points**2} points")
        print(f"  Observation multiplier: {main_result['total_obs']//(num_points**2)}x")
        ...
        try:
            plotting_helper.make_plots(
                X_train,
                [main_result['y_pred']],  # Adjust based on plotting_helper requirements
                X_test,
                main_result['y_pred'],
                true_function,
                X1_grid=X1_grid,
