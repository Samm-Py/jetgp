...
ddgp_lhs_contour.py
------------------------------------------------------------
• 16 Latin–Hypercube points inside a box  [–5,10] × [0,15]
...
def main():
    plot_dir = "contour_example_ECL_plots_branin"
    n_order = 5
    box = ((-5, 10), (0, 15))  # variable bounds for optimizer
    bounds = np.array(box)
    ...
    gx = np.linspace(box[0][0], box[0][1], 20)
    gy = np.linspace(box[1][0], box[1][1], 20)
    ...
    gx = np.linspace(box[0][0], box[0][1], 30)
    gy = np.linspace(box[1][0], box[1][1], 30)
    X1, X2 = np.meshgrid(gx, gy)
    X_pred = np.column_stack([X1.ravel(), X2.ravel()])
