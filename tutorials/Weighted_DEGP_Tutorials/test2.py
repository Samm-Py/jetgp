...
class GroupedSubmodelConfig:
    ...
    n_order: int = 1                     # Maximum derivative order
    n_bases: int = 1                     # Number of input dimensions (1D case)
    lb_x: float = 0.2                    # Lower bound of domain
    ub_x: float = 6.0                    # Upper bound of domain
    num_points: int = 5                 # Number of training points
...
class GroupedSubmodelGP:
    ...
    def generate_training_points(self) -> np.ndarray:
        ...
        X_train = np.array([.6, 1.78, 2.90, 4.40, 5.60]).reshape(-1, 1)
        return X_train
...
def true_function(X, alg=oti):
    ...
    x = X[:, 0]
    return alg.exp(-x) + alg.sin(x) + alg.cos(3*x) + .2*x + 1
...
def main():
    ...
    config = GroupedSubmodelConfig(
        n_order=1,  # Max order needed for any submodel
        num_points=6,
        submodel_groups=[[0, 1, 2], [3, 4]],
        # Order for submodel 1, order for submodel 2
        submodel_orders=[1, 1],
