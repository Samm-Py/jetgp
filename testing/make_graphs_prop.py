...
if __name__ == "__main__":
    ...
    # List to hold legend handles and labels (will be common for all plots)
    global_handles = []
    global_labels = []
    for index_order, order in enumerate(orders):
        n_order = (
            order  # The order of derivative information used by the GP model
        )
        n_bases = 1  # The dimensionality of the function (1D in this demo)
        lb_x = -np.pi  # Lower bound for sampling
        ub_x = np.pi  # Upper bound for sampling
        ...
        if X_train.shape[1] == 1:
            sigma = np.sqrt(abs(np.diag(y_cov)))

            if X_train.shape[1] == 1:
                ax = axes[index_order]
                ax.set_xticks([-2, 0, 2])  # Set x-ticks to -2, 0, 2
                ax.set_yticks([-2, 0, 2])  # Set x-ticks to -2, 0, 2

                # Create a perturbed test set (assuming similar logic as before)
                # ----- Plot Results -----
                ...
                # Compute the true function values for this order
                true_values = true_function(X_test_pert).real

                # Calculate sigma from y_cov (only valid for single-input case)
                sigma = np.sqrt(np.abs(np.diag(y_cov)))

                # Plot the true function (red dashed line)
                (line_true,) = ax.plot(
                    X_test,
                    true_values,
                    "r--",
                ...
                # Plot the training points (black scatter)
                ...
                # Plot the GP mean prediction (blue line)
                (line_pred,) = ax.plot(
                    X_test,
                ...
                # Fill the 95% confidence interval (blue shaded area)
                ci = ax.fill_between(
                    X_test.ravel(),
                    y_pred[: X_test.shape[0]]
                    - 1.96 * sigma[: X_test.shape[0]],
                    y_pred[: X_test.shape[0]]
                    + 1.96 * sigma[: X_test.shape[0]],
                    color="b",
                    alpha=0.2,
                ...
                # ax.set_title(
                #     f"Order {n_order} Enhanced Gaussian Process", fontsize=12
                # )

                # For the first subplot, collect the handles and labels for the global legend
                if index_order == 0:
                    # ax.set_xlabel("x", fontsize=12)
                    global_handles.extend(
                        [line_true, scatter_train, line_pred, ci]
                    )
...
# # Adjust layout of subplots to make space for the legend on the right
# fig.tight_layout(rect=[0, 0, 0.73, 1.0])

# # Create a single global legend on the right side of the grid
# fig.legend(
#     global_handles,
#     global_labels,
#     loc="upper right",
