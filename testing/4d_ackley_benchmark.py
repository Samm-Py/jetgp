...
if __name__ == "__main__":
    ...
    def true_function(X, alg=oti):
        ...
        f = (
            10 * 4  # base constant term: 10 * number of dimensions
            + x1**2 - 10 * alg.cos(2 * np.pi * x1)
            + x2**2 - 10 * alg.cos(2 * np.pi * x2)
            + x3**2 - 10 * alg.cos(2 * np.pi * x3)
            + x4**2 - 10 * alg.cos(2 * np.pi * x4)
        )
    ...
    lower_bounds = [-1 for _ in range(4)]
    upper_bounds = [1 for _ in range(4)]
       ...
       # ----- Generate Training Data -----
        ...
        for pt in pts:
            ...
            params = gp.optimize_hyperparameters(
                n_restart_optimizer=15, swarm_size=250
            )
            print(params)
    ...
    plt.savefig("rmse_plot_4d_ackley.pdf", bbox_inches="tight")  # Save as PDF
    ...
    plt.savefig("max_error_plot_4d_ackley.pdf",
                bbox_inches="tight")  # Save as PDF

    plt.tight_layout()
    plt.show()
    with open("4d_rmse_benchmark_ackley_data.pkl", "wb") as f:
        pickle.dump(rmse_data, f)

    with open("4d_max_error_benchmark_ackley_data.pkl", "wb") as f:
        pickle.dump(rmse_data, f)

    with open("4d_rmse_benchmark_ackley_data_pts.pkl", "wb") as f:
