...
if __name__ == "__main__":
    ...
    def true_function(x1, x2, x3, a=7, b=.1, alg=oti):
    ...
    X_test = np.array([.654, .165, -.948]).reshape(1, -1)
    x1_real = X_test[0, 0]
    x2_real = X_test[0, 1]
    x3_real = X_test[0, 2]
    for order in range(0, 6):
        ...
        with open("3d_rmse_benchmark_ishigami_data.pkl", "rb") as f:
            rmse_data = pickle.load(f)

        with open("3d_rmse_benchmark_ishigami_data_pts.pkl", "rb") as f:
        ...
        def time_hc():
            true_function(x1_im, x2_im, x3_im, alg=oti)

        # Timing function for numpy
        def time_np():
            true_function(x1_real, x2_real, x3_real, alg=np)
    ...
    plt.savefig("time_plot_3d_ishigami_benchmark.pdf",
