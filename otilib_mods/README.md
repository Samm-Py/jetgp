...
- **`build_static.py`** — Builds a single Cython static module at `-O1` (e.g. `python build_static.py m10n2`).
...
- **`src_CMakeLists.txt`** — Adds auto-detected `make mXnY` targets with `-fopenmp` and `-O1` for all `onummXnY` C libraries.
- **`src_python_pyoti_CMakeLists.txt`** — Changes the Cython build command from `python` to `python3` and compiles static Cython module targets at `-O1`.
...
> **Note:** All hardcoded paths in the build scripts are rewritten automatically by `setup_otilib`. `setup_otilib` also patches the top-level otilib release C flags and Cython `setup.py.in` template from `-O3` to `-O1`.
