...
#*****************************************************************************************************
cpdef empty( object shape_in, bases_t nbases = 0, ord_t order = 0, uint64_t nip = 0):
  """
  PURPOSE: Create uninitialized matrix of shape: (nrows,ncols).
           Like zeros() but skips zero-initialization for performance.
           Use when the array will be fully overwritten before reading.
  ...
  if nip == 0:
    ...
    # This will create an {arr_pytype}.
    Ores = {arr_func}_createEmpty(nrows,ncols)
    ...
    # This will create a {fearr_pytype}
    Fres = {fearr_func}_createEmpty(nrows,ncols,nip)
