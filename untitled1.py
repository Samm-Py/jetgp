...
X_train = np.vstack([X_all[submodel1_indices], X_all[submodel2_indices]])
...
submodel_data = [
    [y_vals, d1_sm1, d2_sm1],
    [y_vals, d1_sm2, d2_sm2]
]
...
params = model.optimize_hyperparameters(
    optimizer='jade', pop_size=100, n_generations=15
)
...
axes[0].fill_between(X_test.flatten(),
                     y_pred_sm1 - 1.96 * std_sm1,
                     y_pred_sm1 + 1.96 * std_sm1,
                     color='gray', alpha=0.3, label='95% CI')
axes[0].scatter(X_train[:5], y_vals[:5], c='black', s=100, marker='o', label='Function values')
axes[0].scatter(X_train[:5], y_vals[:5], c='red', s=100, marker='^', label='Function + derivatives')
axes[0].set_xlabel('x')
axes[0].set_ylabel('y')
...
axes[1].fill_between(X_test.flatten(),
                     y_pred_sm2 - 1.96 * std_sm2,
                     y_pred_sm2 + 1.96 * std_sm2,
                     color='gray', alpha=0.3, label='95% CI')
axes[1].scatter(X_train[5:], y_vals[5:], c='black', s=100, marker='o')
axes[1].scatter(X_train[5:], y_vals[5:], c='red', s=100, marker='^')
axes[1].set_xlabel('x')
axes[1].set_ylabel('y')
...
axes[2].fill_between(X_test.flatten(),
                     ...
                     y_pred.flatten() + 1.96 * std_global,
                     color='gray', alpha=0.3, label='95% CI')
axes[2].scatter(X_train, y_vals, c='black', s=100, marker='o')
axes[2].scatter(X_train, y_vals, c='red', s=100, marker='^')
axes[2].set_xlabel('x')
axes[2].set_ylabel('y')
...
output_path = Path("./docs/source/_static/wdegp_sin_example.png")
