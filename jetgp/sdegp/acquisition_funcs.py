...
def find_next_point_batch(
    ...
    **acq_kwargs
    ...
    for n in range(n_batch_points):
        ...
        precomputed_diffs = {
            'train_cand': sdegp_utils.differences_by_dim_func(
                gp[0].x_train,candidate_points_norm, gp[0].n_order
            ),
            'cand_train': sdegp_utils.differences_by_dim_func(
                candidate_points_norm,gp[0].x_train, gp[0].n_order
            ),
            'cand_cand': sdegp_utils.differences_by_dim_func(
                candidate_points_norm, candidate_points_norm, gp[0].n_order
            ),
            'domain_cand': sdegp_utils.differences_by_dim_func(
                integration_points_norm, candidate_points_norm, gp[0].n_order
            ),
            'train_domain': sdegp_utils.differences_by_dim_func(
        ...
        if local_opt:
            ...
            def objective_function(x: np.ndarray) -> float:
                ...
                diff_train_cand = sdegp_utils.differences_by_dim_func(
                     gp[0].x_train,x_cand_norm,  gp[0].n_order
                )
                diff_cand_train = sdegp_utils.differences_by_dim_func(
                     x_cand_norm,gp[0].x_train,  gp[0].n_order
                )
                diff_cand_cand = sdegp_utils.differences_by_dim_func(
                    x_cand_norm, x_cand_norm, gp[0].n_order
                )
                diff_domain_cand = sdegp_utils.differences_by_dim_func(
...
def mse_reduction(
    ...
    weights_matrix = sdegp_utils.determine_weights(
    ...
    for i in range(len(submodel_indices)):
        ...
        K_s = sdegp_utils.rbf_kernel_predictions(
        ...
        K_ss = sdegp_utils.rbf_kernel_predictions(
...
def imse_reduction(
    ...
    K_train_cand = sdegp_utils.rbf_kernel(
        diff_train_cand, length_scales, gp.n_order, gp.n_bases, gp.kernel_func,
        gp.flattened_der_indicies, gp.powers, return_deriv=return_deriv
    )
    
    K_cand_cand = np.diag(sdegp_utils.rbf_kernel(
        diff_cand_cand, length_scales, gp.n_order, gp.n_bases, gp.kernel_func,
        gp.flattened_der_indicies, gp.powers, return_deriv=return_deriv
    ))
    

    K_domain_cand = sdegp_utils.rbf_kernel( diff_domain_cand, length_scales, gp.n_order, gp.n_bases, gp.kernel_func, gp.flattened_der_indicies, gp.powers, return_deriv=return_deriv ) 
