...
Sliced GEK-style variant of WDEGP. Training points are partitioned into
m slices along a chosen coordinate; the likelihood is approximated as a
signed sum of pair-block and single-slice-block log-likelihoods
(Cheng & Zimmermann 2024, eq. 20), with zero-mean GP so the variance
term collapses into the standard 0.5 y^T K^-1 y data fit.

Submodel Types (inherited from WDEGP):
...
class sdegp:
    ...
    def __init__(
        ...
        num_submodels = len(y_train)
        if submodel_weights is None:
            self.submodel_weights = np.ones(num_submodels, dtype=np.float64)
        else:
            w = np.asarray(submodel_weights, dtype=np.float64)
            if w.shape != (num_submodels,):
                raise ValueError(
                    f"submodel_weights must have shape ({num_submodels},), got {w.shape}"
                )
            self.submodel_weights = w
    ...
    def predict(
        ...
        if self.num_submodels > 1:
            if self.submodel_type == 'degp':
                from jetgp.sdegp import sdegp_utils
                diffs_for_weights = sdegp_utils.differences_by_dim_func(
    ...
    def _compute_train_test_differences(self, x_train, X_test, return_deriv, rays_predict,
        ...
        if self.submodel_type == 'degp':
            from jetgp.sdegp import sdegp_utils
            return sdegp_utils.differences_by_dim_func(
    ...
    def _compute_weight_differences(self, X_test, x_train):
        ...
        if self.submodel_type == 'degp':
            from jetgp.sdegp import sdegp_utils
            return sdegp_utils.differences_by_dim_func(X_test, x_train, 0,self.oti, return_deriv=False)
        elif self.submodel_type == 'ddegp':
            from jetgp.full_ddegp import wddegp_utils
            return wddegp_utils.differences_by_dim_func(
