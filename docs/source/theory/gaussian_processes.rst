...
The Gaussian Process can then be written as in equation :eq:`eqn3`.
...
* :math:`\theta_i` is the **length scale** for each input dimension :math:`i`. It determines how quickly the correlation between points decreases with distance. A small length scale means the function varies rapidly, while a large length scale indicates a smoother function.

To make predictions using a Gaussian process model, we assume an observed training dataset :math:`\mathcal{D}` containing :math:`n` input-output pairs:

.. math::

   \mathcal{D} = \left\{\mathbf{X}, \mathbf{Y} \right\} = \left\{(\mathbf{x}_i, y_i) \mid i = 1, \ldots, n \right\}

where each input :math:`\mathbf{x}_i \in \mathbb{R}^d` has a corresponding observed output :math:`y_i = f(\mathbf{x}_i)`. For now, we will assume the output of :math:`f` is scalar, though this framework can be generalized to vector outputs, as discussed in Section :ref:`MOGPs`.
...
For notational simplicity, we can partition this covariance matrix into blocks:
...
.. math::
   ...
   \log p(\mathbf{y}|\mathbf{X}, \boldsymbol{\psi}) =
   -\frac{1}{2} \mathbf{y}^\top \mathbf{K}_{\boldsymbol{\psi}}^{-1} \mathbf{y} 
   - \frac{1}{2}\log|\mathbf{K}_{\boldsymbol{\psi}}| 
   - \frac{n}{2}\log 2\pi

where :math:`\mathbf{K}_{\boldsymbol{\psi}} = K(\mathbf{X},\mathbf{X})`. Optimized hyperparameters, :math:`\boldsymbol{\psi}^*`, are then selected according to Equation :eq:`maxMLL`.

.. math::
   :label: maxMLL

   \boldsymbol{\psi}^* = \arg\max_{\boldsymbol{\psi}} \left( \log p(\mathbf{y}|\mathbf{X}, \boldsymbol{\psi}) \right)
