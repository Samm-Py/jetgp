...
.. jupyter-execute::
    ...
    submodel_indices = [
        [[0, 2, 4, 6, 8], [4, 5, 6]],  # Submodel 0: 1st order at 5 pts, 2nd order at 3 pts
        [[1, 3, 5, 7, 9], [3, 4, 5]]   # Submodel 1: 1st order at 5 pts, 2nd order at 3 pts
    ]
...
Example 6: Single Submodel with Different Indices per Derivative Order
======================================================================

This example demonstrates a **single submodel** where **different derivative orders are at different training points**. This is useful when higher-order derivatives are expensive to compute, so you only compute them at a subset of points.

**Scenario:**

- 1st order derivatives at points [1, 2, 3, 4, 5, 6] (6 points)
- 2nd order derivatives at points [3, 4, 5] only (3 points - a subset)

Since this is a single submodel, ``return_deriv=True`` is available.
...
Step 4: Define heterogeneous derivative indices
~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~

.. jupyter-execute::

    # Different indices for different derivative orders
    d1_indices = [1, 2, 3, 4, 5, 6]  # 1st order at 6 points
    d2_indices = [3, 4, 5]           # 2nd order at only 3 points (subset)
    
    # Single submodel with heterogeneous derivative indices
    # Structure: [[indices_for_1st_order, indices_for_2nd_order]]
    submodel_indices = [[d1_indices, d2_indices]]
    
    # Derivative specs for a single submodel with both derivative orders
    derivative_specs = [[[[[1, 1]]], [[[1, 2]]]]]
    
    print(f"Training points: {num_points}")
    print(f"1st order derivative indices: {d1_indices} ({len(d1_indices)} points)")
    print(f"2nd order derivative indices: {d2_indices} ({len(d2_indices)} points)")
    print(f"\nStructure:")
    print(f"  submodel_indices = {submodel_indices}")
    print(f"  derivative_specs = {derivative_specs}")
    print(f"\nThis is a SINGLE submodel, so return_deriv=True IS available!")

**Explanation:**  
The key here is that ``submodel_indices[0][0]`` (1st order indices) and ``submodel_indices[0][1]`` (2nd order indices) are **different lists**. This allows fine-grained control over where each derivative order is computed.

---

Step 5: Compute derivative values
~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~

.. jupyter-execute::

    # Compute derivatives at their respective points
    d1_values = np.array([[f1_fun(X_train[idx, 0])] for idx in d1_indices])
    d2_values = np.array([[f2_fun(X_train[idx, 0])] for idx in d2_indices])
    
    print(f"Data shapes:")
    print(f"  Function values: {y_function_values.shape} (all {num_points} points)")
    print(f"  1st derivatives: {d1_values.shape} ({len(d1_indices)} points)")
    print(f"  2nd derivatives: {d2_values.shape} ({len(d2_indices)} points)")

---

Step 6: Build and optimize the model
~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~

.. jupyter-execute::

    # Submodel data: [function_values, d1_data, d2_data]
...
.. jupyter-execute::
    ...
    y_pred_func = gp_model.predict(X_train, params, calc_cov=False)
    print(f"Max function value error: {np.max(np.abs(y_pred_func.flatten() - y_function_values.flatten())):.2e}")

---

Step 8: Verify derivatives using return_deriv=True
~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~
...
.. jupyter-execute::
    ...
    # Verify 1st derivatives at d1_indices
    X_d1_points = X_train[d1_indices]
    y_pred_d1 = gp_model.predict(X_d1_points, params, calc_cov=False, return_deriv=True)
    
    print(f"\nPrediction shape at d1 points: {y_pred_d1.shape}")
    print("  Row 0: function values")
    print("  Row 1: 1st derivatives")
    print("  Row 2: 2nd derivatives")
    
    pred_d1 = y_pred_d1[1, :]  # 1st derivatives
    
    print(f"\n1st order derivatives at {d1_indices}:")
    for local_idx, global_idx in enumerate(d1_indices):
        analytic = d1_values[local_idx, 0]
        predicted = pred_d1[local_idx]
        rel_error = abs(predicted - analytic) / abs(analytic) if analytic != 0 else abs(predicted - analytic)
        print(f"  Point {global_idx} (x={X_train[global_idx, 0]:.3f}): Pred={predicted:.6f}, Analytic={analytic:.6f}, Rel Error={rel_error:.2e}")
    
    # Verify 2nd derivatives at d2_indices
    X_d2_points = X_train[d2_indices]
    y_pred_d2 = gp_model.predict(X_d2_points, params, calc_cov=False, return_deriv=True)
    
    pred_d2 = y_pred_d2[2, :]  # 2nd derivatives
    
    print(f"\n2nd order derivatives at {d2_indices}:")
    for local_idx, global_idx in enumerate(d2_indices):
        analytic = d2_values[local_idx, 0]
        predicted = pred_d2[local_idx]
        rel_error = abs(predicted - analytic) / abs(analytic) if analytic != 0 else abs(predicted - analytic)
        print(f"  Point {global_idx} (x={X_train[global_idx, 0]:.3f}): Pred={predicted:.6f}, Analytic={analytic:.6f}, Rel Error={rel_error:.2e}")
...
Step 9: Predict derivatives at non-training points
~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~

.. jupyter-execute::

    # Note: 2nd derivatives can be predicted at ANY point, not just training points
    # The GP learns the underlying function and can extrapolate derivatives
    
    non_d2_points = [idx for idx in d1_indices if idx not in d2_indices]
    print("=" * 70)
    print("2nd order derivative predictions at points WITHOUT 2nd order training data:")
    print("=" * 70)
    
    X_non_d2 = X_train[non_d2_points]
    y_pred_non_d2 = gp_model.predict(X_non_d2, params, calc_cov=False, return_deriv=True)
    pred_d2_extra = y_pred_non_d2[2, :]
    
    for local_idx, global_idx in enumerate(non_d2_points):
        analytic = f2_fun(X_train[global_idx, 0])
        predicted = pred_d2_extra[local_idx]
        rel_error = abs(predicted - analytic) / abs(analytic) if analytic != 0 else abs(predicted - analytic)
        print(f"  Point {global_idx} (x={X_train[global_idx, 0]:.3f}): Pred={predicted:.6f}, Analytic={analytic:.6f}, Rel Error={rel_error:.2e}")
    
    print("\nNote: The GP can predict derivatives at any point, but interpolation")
    print("      is only guaranteed at points where derivatives were trained.")
...
.. jupyter-execute::
    ...
    # Subplot 1: Function prediction
    ax1 = axes[0]
    ax1.plot(X_test, y_true, 'b-', label='True Function', linewidth=2)
    ax1.plot(X_test.flatten(), y_pred.flatten(), 'r--', label='GP Prediction', linewidth=2)
    ax1.fill_between(X_test.ravel(),
                     (y_pred.flatten() - 2*np.sqrt(y_cov)).ravel(),
                     (y_pred.flatten() + 2*np.sqrt(y_cov)).ravel(),
                     color='red', alpha=0.3, label='95% Confidence')
    ax1.scatter(X_train, y_function_values, color='black', s=50, label='All Training Points')
    ax1.scatter(X_train[d1_indices], y_function_values[d1_indices], 
                color='green', s=100, marker='o', label=f'1st order pts', zorder=5)
    ax1.scatter(X_train[d2_indices], y_function_values[d2_indices],
                color='purple', s=150, marker='s', label=f'2nd order pts', zorder=6)
    ax1.set_title("Single Submodel, Heterogeneous Derivative Indices")
    ax1.set_xlabel("x")
    ax1.set_ylabel("f(x)")
    ax1.legend(fontsize=8)
    ax1.grid(alpha=0.3)
    
    # Subplot 2: Derivative coverage visualization
    ax2 = axes[1]
    ax2.bar(range(num_points), [1 if i in d1_indices else 0 for i in range(num_points)], 
            alpha=0.5, label='Has 1st order', color='green')
    ax2.bar(range(num_points), [0.5 if i in d2_indices else 0 for i in range(num_points)], 
            alpha=0.7, label='Has 2nd order', color='purple', 
            bottom=[1 if i in d1_indices else 0 for i in range(num_points)])
    ax2.set_xlabel("Training Point Index")
    ax2.set_ylabel("Derivative Orders Present")
    ax2.set_title("Derivative Coverage per Training Point")
    ax2.legend()
...
Summary
~~~~~~~~~
This example demonstrates a **single submodel with heterogeneous derivative indices**:

**Key takeaways:**

1. **Different indices per derivative order**: ``submodel_indices = [[d1_indices, d2_indices]]`` where the two lists are different
2. **Cost-effective design**: Compute expensive higher-order derivatives only where needed
3. **return_deriv=True available**: Single submodels always support direct derivative predictions
4. **Extrapolation capability**: GP can predict derivatives at any point, but interpolation is only guaranteed at training points
5. **Data structure**: ``submodel_data = [[func_values, d1_data, d2_data]]`` where data arrays have different lengths

Example 7: 2D Single Submodel with Heterogeneous Derivative Coverage
====================================================================

This example demonstrates a **single submodel** in 2D where different spatial regions have different derivative coverage:

- **Boundary points**: Only 1st order derivatives (:math:`\frac{\partial f}{\partial x_1}`, :math:`\frac{\partial f}{\partial x_2}`)
- **Interior points**: Both 1st and 2nd order derivatives

This is useful when higher-order derivatives are expensive or unavailable at boundary regions.
...
Step 4: Generate grid and identify boundary/interior points
~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~

.. jupyter-execute::

    def create_heterogeneous_grid(lb=-1.0, ub=1.0, num_points=5):
        """
        Create a 2D grid with different derivative coverage:
        - Boundary: 1st order only (df/dx1, df/dx2)
        - Interior: 1st and 2nd order (df/dx1, df/dx2, d²f/dx1², d²f/dx2²)
        """
        ...
        # Identify boundary and interior indices
        boundary_indices = []
        interior_indices = []
        ...
        # All derivative points (boundary + interior have 1st order)
        all_deriv_indices = boundary_indices + interior_indices
        
        # Build submodel_indices:
        # - 1st order (df/dx1, df/dx2): at ALL derivative points
        # - 2nd order (d²f/dx1², d²f/dx2²): only at INTERIOR points
        submodel_indices = [
            [all_deriv_indices,   # [0] df/dx1 at all points
             all_deriv_indices,   # [1] df/dx2 at all points
             interior_indices,    # [2] d²f/dx1² only at interior
             interior_indices]    # [3] d²f/dx2² only at interior
        ]
        
        # Derivative specs (4 derivatives total)
        derivative_specs = [
            [
                [[[1, 1]],  # df/dx1
                 [[2, 1]]],  # df/dx2
        ...
        return X, submodel_indices, derivative_specs, boundary_indices, interior_indices, all_deriv_indices

    X_train, submodel_indices, derivative_specs, boundary_idx, interior_idx, all_deriv_idx = create_heterogeneous_grid()
    
    print(f"Total training points: {len(X_train)}")
    print(f"Boundary points: {len(boundary_idx)} -> {boundary_idx}")
    print(f"Interior points: {len(interior_idx)} -> {interior_idx}")
    print(f"\nDerivative coverage:")
    print(f"  df/dx1:    {len(submodel_indices[0][0])} points (boundary + interior)")
    print(f"  df/dx2:    {len(submodel_indices[0][1])} points (boundary + interior)")
    print(f"  d²f/dx1²:  {len(submodel_indices[0][2])} points (interior only)")
    print(f"  d²f/dx2²:  {len(submodel_indices[0][3])} points (interior only)")

---

Step 5: Visualize derivative coverage
~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~

.. jupyter-execute::

    def visualize_heterogeneous_coverage(X_train, boundary_idx, interior_idx):
        fig, ax = plt.subplots(figsize=(8, 8))
        
        # Boundary points (1st order only)
        ax.scatter(X_train[boundary_idx, 0], X_train[boundary_idx, 1],
                   c='blue', s=120, edgecolor='k', marker='o', label='1st order only (boundary)')
        
        # Interior points (1st + 2nd order)
        ax.scatter(X_train[interior_idx, 0], X_train[interior_idx, 1],
                   c='red', s=150, edgecolor='k', marker='s', label='1st + 2nd order (interior)')
        
        # Label points
        for i, p in enumerate(X_train):
            ax.text(p[0] + 0.05, p[1] + 0.05, str(i), fontsize=8)
        
        ax.set_title("Heterogeneous Derivative Coverage\n(Blue=1st order, Red=1st+2nd order)")
        ax.set_xlabel("$x_1$")
        ax.set_ylabel("$x_2$")
    ...
    visualize_heterogeneous_coverage(X_train, boundary_idx, interior_idx)
...
.. jupyter-execute::
    ...
    def prepare_heterogeneous_data(X_train, submodel_indices, derivative_specs, func, n_order, n_bases):
        y_function_values = func(X_train, alg=np).reshape(-1, 1)
        current_data = [y_function_values]
        for sm_indices, sm_specs in zip(submodel_indices, derivative_specs):


            for deriv_idx, (point_indices, deriv_spec) in enumerate(zip(sm_indices, sm_specs)):
                deriv_data = []
                X_sub = oti.array(X_train[sm_indices[2*deriv_idx]])
                ...
                y_with_derivs = oti.array([func(x, alg=oti)[0] for x in X_sub])
                for spec in deriv_spec:
                    deriv_values = y_with_derivs.get_deriv(spec).reshape(-1, 1)
                    current_data.append(deriv_values)

                deriv_names = ["df/dx1", "df/dx2", "d²f/dx1²", "d²f/dx2²"]
                print(f"  {deriv_names[deriv_idx]}: shape {deriv_values.shape} at {len(point_indices)} points")

        return [current_data]

    print("Data array shapes:")
    submodel_data = prepare_heterogeneous_data(
        X_train, submodel_indices, derivative_specs, six_hump_camelback, n_order, n_bases
    )
...
.. jupyter-execute::
    ...
    params = gp_model.optimize_hyperparameters(
        optimizer='jade', pop_size=100, n_generations=15, debug=True
    )
...
.. jupyter-execute::
    ...
    y_true_grid = y_true.reshape(N_test, N_test)
    y_pred_grid = y_pred.reshape(N_test, N_test)
    error_grid = np.abs(y_true_grid - y_pred_grid)
    
    for ax, data, title, cmap in zip(axes, 
                                      [y_true_grid, y_pred_grid, error_grid],
                                      ["True Function", "GP Prediction", f"Absolute Error (NRMSE={nrmse:.4f})"],
                                      ["viridis", "viridis", "magma"]):
        c = ax.contourf(X1, X2, data, levels=50, cmap=cmap)
        fig.colorbar(c, ax=ax)
        ax.scatter(X_train[boundary_idx, 0], X_train[boundary_idx, 1], c='blue', edgecolor='k', s=60, marker='o')
        ax.scatter(X_train[interior_idx, 0], X_train[interior_idx, 1], c='white', edgecolor='k', s=80, marker='s')
        ax.set_title(title)
        ax.set_xlabel("$x_1$")
        ax.set_ylabel("$x_2$")
    
    plt.suptitle("Example 7: 2D Heterogeneous Derivative Coverage", fontsize=14)
    plt.tight_layout()
    plt.show()
...
Summary
~~~~~~~~~
**Key insight**: Different derivatives can have different spatial coverage within a single submodel:

.. code-block:: python

   submodel_indices = [
       [all_points,      # df/dx1 everywhere
        all_points,      # df/dx2 everywhere
        interior_only,   # d²f/dx1² only at interior
        interior_only]   # d²f/dx2² only at interior
   ]

This is useful when:

- Higher-order derivatives are expensive to compute
- Boundary conditions only provide lower-order derivative information
- You want to concentrate computational effort in regions of interest
