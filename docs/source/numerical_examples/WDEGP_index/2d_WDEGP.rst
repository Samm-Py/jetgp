...
uses the same uniform set of derivatives. Function values from all training points
are used to inform the global fit, but derivative information is only computed
for a subset of points specified in `submodel_point_groups`.
...
- **Uniform Derivative Structure**: Every submodel uses the same set of derivatives.
- **Automatic Data Reordering**: Training data is reordered to satisfy the sequential indexing requirements of the GP framework.
...
.. jupyter-execute::
    ...
    # User-defined point groups for derivatives
    submodel_point_groups = [
        [0, 3, 12, 15],       # Corners
        [1, 2, 4, 8, 7, 11, 13, 14],  # Edges
        [5, 6, 9, 10]         # Center
    ]
        ...
        for k, point_indices in enumerate(submodel_indices):
            ...
            for i in range(len(base_der_indices)):
                for j in range(len(base_der_indices[i])):
                    deriv = y_with_derivatives.get_deriv(base_der_indices[i][j]).reshape(-1, 1)
...
.. jupyter-execute::
    ...
    print("Total training points:", X_train_initial.shape[0])
    print("Points used for derivatives:", len(list(itertools.chain.from_iterable(submodel_point_groups))))
    print("Number of submodels:", len(submodel_point_groups))
...
.. jupyter-execute::
    ...
    y_true_grid = y_true.reshape(test_points_per_axis, test_points_per_axis)
    y_pred_grid = y_pred.reshape(test_points_per_axis, test_points_per_axis)
    abs_error_grid = abs_error.reshape(test_points_per_axis, test_points_per_axis)
