...
class TestWDEGP1DIndividual(unittest.TestCase):
    ...
    def setUpClass(cls):
        ...
        cls.derivative_specs = [
            cls.base_derivative_indices for _ in range(cls.num_points)]
    ...
    def _compute_symbolic_derivatives(cls):
        ...
        for k, idx in enumerate(cls.submodel_indices):
            x_point = cls.X_train[idx[0]].flatten()[0]
    ...
    def test_function_value_interpolation(self):
        ...
        y_pred_train, _ = self.model.predict(
            self.X_train, self.params, calc_cov=True)

        abs_errors = np.abs(y_pred_train.flatten() -
    ...
    def test_first_derivative_interpolation(self):
        ...
        for i, idx in enumerate(self.submodel_indices):
            x_point = self.X_train[idx[0]].flatten()[0]
    ...
    def test_second_derivative_interpolation(self):
        ...
        for i, idx in enumerate(self.submodel_indices):
            ...
            X_center = self.X_train[idx[0]].reshape(1, -1)
    ...
    def test_comprehensive_summary(self):
        ...
        status = "✓ PASS" if passed else "✗ FAIL"
        print(
            f"{status} | First derivatives | Max: {max_first_error:.2e} | Mean: {mean_first_error:.2e}")
        ...
        print(
            f"  - Submodels: {len(self.submodel_data)} (one per training point)")
