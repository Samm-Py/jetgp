...
class TestSparseWDEGP1D(unittest.TestCase):
    ...
    def setUpClass(cls):
        ...
        cls.X_train = np.linspace(
            cls.lb_x, cls.ub_x, cls.num_points).reshape(-1, 1)
        ...
        cls.base_derivative_indices = utils.gen_OTI_indices(
            cls.n_bases, cls.n_order)
    ...
    def test_function_value_interpolation(self):
        ...
        y_pred_train = self.gp_model.predict(
            self.X_train, self.params, calc_cov=False)

        errors = np.abs(y_pred_train.flatten() -
                        self.y_function_values.flatten())
    ...
    def test_confidence_intervals(self):
        ...
        y_pred, y_cov = self.gp_model.predict(
        ...
        distances_to_train = np.min([np.abs(X_test - x)
                                    for x in self.X_train], axis=0)
    ...
    def test_comprehensive_summary(self):
        ...
        print(
            f"  - Sparse derivative points: {len(self.derivative_indices)} {self.derivative_indices}")
        ...
        y_pred_train = self.gp_model.predict(
            self.X_train, self.params, calc_cov=False)
        func_errors = np.abs(y_pred_train.flatten() -
                             self.y_function_values.flatten())
        ...
        for idx, train_idx in enumerate(self.derivative_indices):
            ...
            first_deriv_errors.append(
                abs(predicted_first - self.d1_all[:, 0].flatten()))
            second_deriv_errors.append(
                abs(predicted_second - self.d2_all[:, 0].flatten()))
