"""
Unit test for the Directional-Derivative Enhanced Gaussian Process (DDEGP)
on the 2D Branin function using a global set of directional rays.

This test verifies that the DDEGP model correctly interpolates:
...
class TestGlobalDirectionalBranin(unittest.TestCase):
    """Test case for DDEGP with global directional rays on the Branin function."""
    
    @classmethod
    def setUpClass(cls):
        """Setup global configuration and training data for the DDEGP test."""
    ...
    def test_shapes(self):
        ...
        self.assertEqual(self.X_train.shape, (self.num_training_pts, 2),
                        "Training points should be 16×2")
    ...
    def test_comprehensive_summary(self):
        ...
        print("\n" + "="*80)
        print("DDEGP Global Directional Rays (Branin Function) Test Summary")
        print("="*80)
        ...
        for i, (y_pred, y_true, angle) in enumerate(zip(y_pred_rays, y_true_rays, ray_angles)):
            ...
            all_tests_passed = all_tests_passed and passed
            status = "✓ PASS" if passed else "✗ FAIL"
            print(f"{status} | Ray {i+1} ({angle:3d}°) | Max: {max_err:.2e} | Mean: {mean_err:.2e}")
        
        print("\n" + "="*80)
        print("Configuration:")
        print(f"  - Function: 2D Branin benchmark function")
        print(f"  - Training points: {self.num_training_pts} (Latin Hypercube)")
        print(f"  - Domain: x1 ∈ [{self.domain_bounds[0][0]}, {self.domain_bounds[0][1]}], "
              f"x2 ∈ [{self.domain_bounds[1][0]}, {self.domain_bounds[1][1]}]")
        print(f"  - Directional rays: {self.num_rays} (45°, 90°, 135°)")
...
def run_tests_with_details():
    ...
    suite = unittest.TestLoader().loadTestsFromTestCase(TestGlobalDirectionalBranin)
...
if __name__ == "__main__":
    print("\nRunning DDEGP Global Directional Rays Interpolation Unit Tests...")
    print("="*80 + "\n")
