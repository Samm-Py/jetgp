...
This tutorial demonstrates a **spatially-aware derivative assignment strategy** 
for Derivative-Enhanced Gaussian Processes (DEGPs). Instead of providing all 
derivatives at every training point, we assign exactly **one derivative per point**, 
cycling through dimensions along a spatial chain.
...
In high-dimensional problems, providing full derivative information at every 
training point can be computationally expensive. The covariance matrix size 
scales with the total number of observations, so reducing derivative observations 
can significantly improve training time and memory usage.

**Key insight:** Gaussian Processes have a correlation structure that allows 
nearby points to share information. If neighboring points have complementary 
derivative information, the GP may be able to "fill in" the missing derivatives 
...
1. **Build a spatial chain:** Connect training points via a nearest-neighbor path
2. **Cycle derivatives along the chain:** Assign derivatives in a repeating pattern
...
This ensures that adjacent points in the chain have different derivative 
information, maximizing the complementary coverage.
...
includes the dimensionality of the input space, grid size, kernel type, and 
...
We use a 2D function with interesting structure that benefits from derivative 
information:
...
The spatial chain connects training points via a greedy nearest-neighbor 
traversal. Starting from point 0, we iteratively add the nearest unvisited 
point to form a connected chain that traverses the space smoothly.
...
Derivatives are assigned by cycling through dimensions based on chain position.
Each point receives exactly one derivative observation.
...
We generate training points on a regular grid and evaluate the function with 
...
We define two setup functions: one for the cycling strategy (1 derivative per 
point) and one for the full strategy (all derivatives at all points) as a baseline.
...
We verify that the model correctly interpolates training data, checking both
function values and derivative values at their specified locations.
...
We create visualizations to understand the derivative assignment pattern and 
...
Finally, we run the full workflow: generate training data, build the spatial 
chain, assign cycling derivatives, train both models, and compare results.
...
+------------+-----------------+----------------+--------------------+
| Strategy   | Function Values | Derivative Obs | Total Observations |
+============+=================+================+====================+
| Cycling    | 25              | 25 (1 per pt)  | 50                 |
+------------+-----------------+----------------+--------------------+
| Full       | 25              | 50 (2 per pt)  | 75                 |
+------------+-----------------+----------------+--------------------+
...
For a d-dimensional problem, the cycling pattern extends naturally. Each point
in the chain receives one derivative, cycling through all dimensions:
...
1. **Spatial correlation enables information sharing:** The GP correlation 
   structure allows derivative information to propagate between neighboring points.

2. **Strategic placement matters:** By cycling derivatives along a spatial chain, 
   neighboring points have complementary information.

3. **Trade-off:** The cycling strategy reduces computational cost at the potential 
   expense of some accuracy. The effectiveness depends on:
